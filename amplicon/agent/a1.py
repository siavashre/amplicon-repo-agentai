...
from amplicon.know_how import KnowHowLoader
from amplicon.llm import SourceType, get_llm
from amplicon.model.retriever import ToolRetriever
from amplicon.tool.support_tools import run_python_repl
from amplicon.tool.tool_registry import ToolRegistry
from amplicon.utils import (
...
class A1:
    def __init__(
        ...
        if commercial_mode:
            from amplicon.env_desc_cm import data_lake_dict, library_content_dict

            print("🏢 Commercial mode: Using commercial-licensed datasets only")
        else:
            from amplicon.env_desc import data_lake_dict, library_content_dict
        ...
        _data_path = os.getenv("AMPLICON_PATH") or os.getenv("AMPLICON_DATA_PATH") or default_config.path
        _amplicon_dir = os.path.join(_data_path, "amplicon_data", "data_lake", "amplicon_data")
        ...
        benchmark_dir = os.path.join(path, "amplicon_data", "benchmark")
        data_lake_dir = os.path.join(path, "amplicon_data", "data_lake")

        # Create the amplicon_data directory structure
        ...
        self.path = os.path.join(path, "amplicon_data")
    ...
    def add_tool(self, api):
        ...
        try:
            ...
            if not hasattr(builtins, "_amplicon_custom_functions"):
                builtins._amplicon_custom_functions = {}
            builtins._amplicon_custom_functions[schema["name"]] = api
    ...
    def add_mcp(self, config_path: str | Path = "./tutorials/examples/mcp_config.yaml") -> None:
        ...
        the amplicon agent system. Each MCP server is loaded as an independent module
        ...
        for server_name, server_meta in mcp_servers.items():
            ...
            for tool_meta in tools_config:
                if isinstance(tool_meta, dict) and "amplicon_name" in tool_meta:
                    # Manual tool definition
                    tool_name = tool_meta.get("amplicon_name")
    ...
    def remove_custom_tool(self, name):
        ...
        if hasattr(builtins, "_amplicon_custom_functions") and name in builtins._amplicon_custom_functions:
            del builtins._amplicon_custom_functions[name]
  ...
  - Bash scripts and commands: Use the #!BASH marker in your execute block for both simple commands and complex shell scripts with variables, loops, conditionals, etc.
    ...
    def configure(self, self_critic=False, test_time_scale_round=0):
        ...
        def execute(state: AgentState) -> AgentState:
            ...
            if execute_match:
                ...
                execution_plots = []
                try:
                    from amplicon.tool.support_tools import get_captured_plots
    ...
    def _log_turn(self, logger, turn_counter, message):
        ...
        if not self.log_tokens or self._last_response is None or self._last_input_messages is None:
            return None
        from amplicon.llm import extract_usage_metadata
    ...
    def go(self, prompt):
        ...
        if self.log_tokens:
            from amplicon.utils import TokenLogger
    ...
    def update_system_prompt_with_selected_resources(self, selected_resources):
        ...
        for tool in selected_resources["tools"]:
            ...
            if isinstance(tool, dict):
                ...
                if not module_name:
                    module_name = "amplicon.tool.scRNA_tools"  # Default to scRNA_tools as a fallback
                    tool["module"] = module_name
            else:
    ...
    def create_mcp_server(self, tool_modules=None):
        """
        Create an MCP server object that exposes internal Amplicon tools.
            ...
            try:
                ...
                for tool_schema in module_tools:
                    ...
                    try:
                        ...
                        wrapper_func = self._generate_mcp_wrapper_from_amplicon_schema(
    ...
    def _generate_markdown_content(self, include_images: bool = True) -> str:
        ...
        content = """# Amplicon Agent Conversation History
...
"""
    ...
    def _clear_execution_plots(self):
        ...
        """
        try:
            from amplicon.tool.support_tools import clear_captured_plots

            clear_captured_plots()
        except Exception as e:
            print(f"Warning: Could not clear execution plots: {e}")

    def _generate_mcp_wrapper_from_amplicon_schema(self, original_func, func_name, required_params, optional_params):
        """Generate wrapper function based on Amplicon schema format."""
        import inspect
    ...
    def launch_gradio_demo(self, thread_id=42, share=False, server_name="0.0.0.0", require_verification=False):
        ...
        available_access_codes = ["Amplicon2025"]
        ...
        with gr.Blocks() as demo:
            ...
            with verification_container:
                gr.Markdown("# Amplicon A1 Agent - Access Verification")
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        main_chatbot = gr.Chatbot(
                            label="Amplicon A1 Agent",
                            height=800,
                            show_copy_button=True,
                        )
                    with gr.Column(scale=1):
                        innerloop_chatbot = gr.Chatbot(
                            label="Amplicon Executor",
                            height=800,
                            show_copy_button=True,
                        )
