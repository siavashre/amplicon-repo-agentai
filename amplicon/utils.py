...
def api_schema_to_langchain_tool(api_schema, mode="generated_tool", module_name=None):
    if mode == "generated_tool":
        module = importlib.import_module("amplicon.tool.generated_tool." + api_schema["tool_name"] + ".api")
...
def read_module2api():
    fields = [
        "amplicon_table", "amplicon_gene", "amplicon_sv", "amplicon_cycle", "amplicon_graph", "gene_annotation"
        # "literature",
        # "biochemistry",
        # "bioimaging",
        # "bioengineering",
    ...
    module2api = {}
    for field in fields:
        module_name = f"amplicon.tool.tool_description.{field}"
        module = importlib.import_module(module_name)
        module2api[f"amplicon.tool.{field}"] = module.description
    return module2api
...
def parse_tool_calls_from_code(code: str, module2api: dict, custom_functions: dict = None) -> list[str]:
    ...
    Example:
        >>> code = "from amplicon.tool import analyze_data\nimport pandas as pd"
...
def inject_custom_functions_to_repl(custom_functions: dict):
    ...
    if custom_functions:
        ...
        from amplicon.tool.support_tools import _persistent_namespace
        ...
        builtins._amplicon_custom_functions.update(custom_functions)
...
def format_detected_tools(detected_tool_modules: list) -> str:
    ...
    Example:
        >>> format_detected_tools([("analyze_data", "amplicon.tool"), ("pandas", "pandas")])
        "amplicon → analyze_data, pandas → pandas"
    """
    tool_descriptions = []
...
def remove_emojis_from_text(text: str) -> str:
    ...
    Note:
        The function targets specific emojis used in the Amplicon system:
        - 🔧 for tools
        - 📊 for data
        - ⚙️ for software
        - 📋 for configuration
...
def convert_markdown_to_pdf(markdown_path: str, pdf_path: str) -> None:
    ...
    try:
        ...
        <head>
            <meta charset="utf-8">
            <title>Amplicon Conversation History</title>
