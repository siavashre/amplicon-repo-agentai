"""
AmpliconEval1: Evaluation loader for Amplicon tasks

This class provides a unified interface to evaluate user answers against ground truth
for all tasks in the AmpliconEval1 benchmark.
"""
...
class AmpliconEval1:
    """
    Evaluation loader for AmpliconEval1 benchmark

    Usage:
        evaluator = AmpliconEval1('biomni_eval1_dataset.parquet')
        score = evaluator.evaluate('gwas_causal_gene_opentargets', 0, 'BRCA1')
    """

    def __init__(self):
        """
        Initialize the AmpliconEval1 evaluator
        ...
        print(f"Loaded AmpliconEval1 dataset: {len(self.df)} instances across {self.df['task_name'].nunique()} tasks")
    ...
    def __repr__(self):
        return f"AmpliconEval1(instances={len(self.df)}, tasks={self.df['task_name'].nunique()})"
...
def main():
    """Demo usage of AmpliconEval1"""
    evaluator = AmpliconEval1()
