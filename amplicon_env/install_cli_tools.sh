...
if [ -n "$AMPLICON_TOOLS_DIR" ]; then
    TOOLS_DIR="$AMPLICON_TOOLS_DIR"
else
...
echo "# Added by amplicon setup" >> "$TOOLS_DIR/setup_path.sh"
    ...
    if [ -n "$profile_file" ]; then
        ...
        if [ -w "$profile_file" ]; then
            ...
            if ! grep -q "export PATH=\"$TOOLS_DIR/bin:\$PATH\"" "$profile_file"; then
                ...
                echo "" >> "$profile_file"
                echo "# Added by amplicon setup" >> "$profile_file"
...
install_cli_tools() {
    ...
    if [ -n "$AMPLICON_AUTO_INSTALL" ]; then
        install_all_tools 1
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    # Set AMPLICON_AUTO_INSTALL if it's not already set
    export AMPLICON_AUTO_INSTALL=1
    ...
    exit 0
elif [ -n "$AMPLICON_AUTO_INSTALL" ]; then
    # Run in auto-install mode without showing the menu
    install_all_tools 1
